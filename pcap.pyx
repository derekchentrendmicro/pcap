...
cdef extern from "pcap.h":
    ...
    void    pcap_dump(u_char *user, pcap_pkthdr *h, u_char *sp)
...
cdef class pcap:
    ...
    def __init__(self, name=None, snaplen=65535, promisc=True,
                 timeout_ms=0, immediate=False, rfmon=False, dump=False):
        global dltoff
        cdef char *p

        if dump:
            self.__pcap = pcap_open_dead(DLT_EN10MB, 65535)
            self.__pd = pcap_dump_open(self.__pcap,name)
            return
    ...
    def dump(self, pktdata):
        cdef pcap_pkthdr *hdr=<pcap_pkthdr *>malloc(sizeof(pcap_pkthdr))
        cdef u_char *pkt
        cdef u_char *p
        p = <u_char*>self.__pd
        hdr.ts.tv_sec = 0
        hdr.ts.tv_usec = 0
        hdr.len = len(pktdata)
        hdr.caplen = len(pktdata)
        pcap_dump(p, hdr, pktdata)
